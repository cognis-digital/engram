...
How **engram** fits the wider Cognis suite — a composable set of defensive, analytical
tools that all run on your own hardware and speak plain **JSON** (or an OpenAI-compatible
**`/v1`**). `engram` lives in the **MCP / agent security** cluster.
...
| from | relation | to |
|---|---|---|
| `engram` | JSON findings compose with | [`mcpharden`](https://github.com/cognis-digital/mcpharden), [`c2detect`](https://github.com/cognis-digital/c2detect), [`agentmap`](https://github.com/cognis-digital/agentmap) |
| `engram` | AI add-ins are served `/v1` by | [`edgemesh`](https://github.com/cognis-digital/edgemesh) (your fleet) |
| `engram` | findings can be narrated / reasoned over by | [`humind`](https://github.com/cognis-digital/humind) -> [`agentlex`](https://github.com/cognis-digital/agentlex) |
| `engram` | export to intel formats via | [`stixgen`](https://github.com/cognis-digital/stixgen) (STIX) / [`attackmap`](https://github.com/cognis-digital/attackmap) (ATT&CK) |

## Composition patterns

Everything reads/writes JSON, so tools chain with ordinary pipes; nothing leaves the box.

**1 — chain within the mcp / agent security cluster.** `engram` output feeds the next tool:
```bash
engram ... --format json > out.json          # this tool's findings (see `engram --help`)
...
engram ...                                         # vision / reasoning add-ins light up
```

**3 — export findings to your SOC's formats.**
```bash
engram ... --format json | stixgen from-json > bundle.stix.json   # STIX 2.1
engram ... --format json | attackmap map > attack.json            # ATT&CK techniques
```

**4 — narrate through cognition + agents.** `humind` extracts salience from `engram`'s
output; `agentlex` holds it as KB facts and fires Horn rules to escalate. `engram` slots
