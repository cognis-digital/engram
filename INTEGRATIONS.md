...
**engram** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `engram` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
engram ... --format json | cognis-connect emit --to sigma   # Sigma rules
engram ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
engram ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `engram` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="engram") for rec in records]   # records = your JSON output
