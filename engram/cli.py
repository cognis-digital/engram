...
Examples
--------
    engram remember "User prefers metric units" --tags preference,units
    engram recall "what units does the user want" --limit 5
    engram list --limit 20
    engram get 3
    engram forget 3
    engram stats

The database path resolves to (in order): --db, $ENGRAM_DB, ./engram_memory.sqlite.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=TOOL_NAME,
        description="Durable, model-agnostic long-term memory for AI agents.",
    )
    parser.add_argument(
        "--version",
        action="version",
