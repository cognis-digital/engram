"""Native cognis-connect emit for engram — forward findings to any platform.

Maps engram's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    engram ... --format json | engram-emit --to stix
    engram-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    """Tailor a engram record toward the Finding contract before normalize().
